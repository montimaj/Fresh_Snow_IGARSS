...
% Single address.
% ---------------
\name{Sayantan Majumdar\textsuperscript{1,2} \orcidicon{0000-0002-3539-0147}\,, Praveen K. Thakur\textsuperscript{2}, Ling Chang\textsuperscript{1}, Shashi Kumar\textsuperscript{2} \orcidicon{0000-0002-2442-7143}}
\address{\textsuperscript{1}ITC, University of Twente, Enschede, The Netherlands\\
...
In principle, the three orthogonal axes ($a_x$, $a_y$, $a_z$) of a spheroid represented in the 3D ($x$, $y$, $z$) Cartesian coordinate system correspond to the three dipoles which essentially characterise the dielectric nature of an ice particle \cite[Fig. 6]{Leinss2014}. Furthermore, the depolarisation factor is an important component to model the snow anisotropy which in turn, is used as a parameter in the FSD inversion algorithm based on the copolar phase difference (CPD) approach \cite{Leinss2014}. 

In this work, the CPD method is applied to compute the FSD in the complex alpine terrains of the Beas watershed, northwestern Himalayas. Additionally, due to the extreme geological conditions there exist significant uncertainty sources which are discussed explicitly in the later sections.
...
\BeforeBeginEnvironment{equation}{\vspace{-2.5ex}}
...
\begin{figure*}[htb]
    \centering
    \includegraphics[scale=0.5]{Pictures/All_Wishart.png}
    \vspace{-2ex}
...
%\begin{figure}[htb]
%\centering
%\includegraphics[width=8.5cm]{Pictures/Wishart_New.png}
%\vspace{-2ex}
%\caption{\textbf{(a)} Wishart classified maps and zoomed views for Jan 8, 2016 and %\textbf{(b)} Jun 8, 2017}
%\label{fig:wishart}
%\end{figure}
...
%\begin{figure}[htb]
%\centering
%\includegraphics[width=8.5cm]{Pictures/HA_All.png}
%\vspace{-2ex}
%\caption{\textbf{(a)} Dual-pol H-$\alpha$ plot, Jan \textbf{(b)} Dual-pol H-$\alpha$ plot, Jun and \textbf{(c)} Quad-pol H-$\alpha$ plot, Jan.}
%\label{fig:had}
%\end{figure}
\vspace{-2ex}
...
\begin{figure}[htb]
\centering
\includegraphics[scale=0.3]{Pictures/Graphs1.png}
...
Therefore, it is recommended to incorporate snow density variations along with the quantitative assessment of the ice particle axial ratio for potential future improvements. Furthermore, different windows have been tested for the FSD values only and hence, the ensemble window size effect on the CPD needs to be further investigated. Additionally, optical datasets such as the Sentinel-2 images could be used in conjunction with the SAR imagery to obtain better snow cover maps which delineate dry and wet snowname types.  Apart from this, other fully polarimetric SAR acquisitions offering high penetration capability such as the ALOS2-PALSAR2 L-band data could be used for subsequent studies in this regard.
